...
namespace Biblioteka.Areas.Identity.Pages.Account
{
    ...
    public class LoginWithRecoveryCodeModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            [Display(Name = "Kod odzyskiwanias")]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("Użytkownik z ID '{UserId}' zalogowany za pomocą kodu odzyskiwania.", user.Id);
                return LocalRedirect(returnUrl ?? Url.Content("~/"));
            }
            if (result.IsLockedOut)
            {
                _logger.LogWarning("Użytkownik z ID '{UserId}' ma zablokowane konto.", user.Id);
